...
Graph::Graph(char *filename, char *filename_w, int type) {
  ...
  */
  ...
  vector<unsigned long> out_d(nb_nodes, 0);
  ...
  */
  ...
  out_degrees.resize(nb_nodes);

  degrees[0] = d[0];
  out_degrees[0] = out_d[0];

  for (int i = 1; i < nb_nodes; i++) {
    degrees[i] = degrees[i - 1] + d[i];
    d[i - 1] = 0;
    out_degrees[i] = out_degrees[i-1] + out_d[i];
    out_d[i - 1] = 0;
  }
  d[nb_nodes-1]=0;
  out_d[nb_nodes - 1] = 0;
  ...
  */
  ...
  for (i = 0; i < nb_links; i++) {
    ...
    if (u>0) {
      ...
      unlinks[out_degrees[u-1] + out_d[u]] = v;
    } else {
      links[d[u]] = v;
      unlinks[out_d[u]] = v;
    }
...
Graph::subgraph(vector<int> nodes) {
  ...
  sub.out_degrees.resize(nodes.size());
  ...
  for (unsigned int node = 0; node<nodes.size(); node++) {
    ...
    sub.out_degrees[node] = (node > 0)?sub.out_degrees[node - 1]:0;

    pair<vector<int>::iterator, vector<long double>::iterator > p = neighbors(nodes[node]);
    pair<vector<int>::iterator, vector<long double>::iterator > q = undir_neighbors(nodes[node]);
    ...
    for(int i=0 ; i<nb_out_neighbors(nodes[node]) ; i++){
      if (innodes[*(q.first + i)] != 0) {
        sub.out_degrees[node]++;
        sub.unlinks.push_back(innodes[*(q.first + i)]-1);
      }
    }
  }
