...
class Graph {
 public:
  ...
  vector<unsigned long long> out_degrees;
  vector<int> links;
  vector<int> unlinks;
  ...
  inline int nb_out_neighbors(int node);
  ...
  inline pair<vector<int>::iterator, vector<long double>::iterator > undir_neighbors(int node);
...
inline int
Graph::nb_out_neighbors(int node) {
  assert(node>=0 && node<nb_nodes);

  if (node==0)
    return out_degrees[0];
  else
    return (int)(out_degrees[node]-out_degrees[node-1]);
}
...
inline pair<vector<int>::iterator, vector<long double>::iterator >
Graph::undir_neighbors(int node) {
  assert(node>=0 && node<nb_nodes);
  
  if (node==0)
    return make_pair(unlinks.begin(), weights.begin());
  else if (weights.size()!=0)
    return make_pair(unlinks.begin()+out_degrees[node-1], weights.begin()+out_degrees[node-1]);
  else
    return make_pair(unlinks.begin()+out_degrees[node-1], weights.begin());
}
