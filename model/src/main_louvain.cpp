...
multiply_and_add(Graph g, vector<int> pnodes, vector<int> cnodes, vector<vector<double>>& embedding, unordered_map<int, vector<double>> embed2, unsigned int step) {
  ...
  vector<double> new_embed(dimension);
  for (unsigned int i = 0; i < dimension; i++)
    new_embed[i] = (double)rand() / RAND_MAX;
  ...
  for(int i=0; i<cnodes.size(); i++){
    ...
    for(int j=0; j < new_embed.size(); j++){
      vector<int>::iterator it = find(pnodes.begin(), pnodes.end(), node);
      if(it != pnodes.end()){
        if( embed2.find(node) != embed2.end() )
          embedding[node][j] += al * (0.8 * new_embed[j] + 0.2 * embed2[node][j]);
        else
          embedding[node][j] += al * new_embed[j];
      }
      else{
        if( embed2.find(node) != embed2.end() )
          embedding[node][j] += al * (0.8 * al * new_embed[j] + 0.2 * embed2[node][j]);
        else
          embedding[node][j] += al * al * new_embed[j];
      }
    }
  }
}
...
h_louvain(Graph g, unsigned int step, vector<int> nodes, vector<vector<double>> & embedding) {
  ...
  for(unsigned int i=0; i<parts.size(); i++){
    ...
    for(unsigned int j=0; j<parts[i].size(); j++){
      ...
      pair<vector<int>::iterator, vector<long double>::iterator > p = g.neighbors(parts[i][j]);

      for (int k=0 ; k<g.nb_neighbors(parts[i][j]) ; k++){
        if( ((double)rand() / RAND_MAX) < 0.3){
          node.insert(*(p.first + k));
          cnodes.insert(nodes[*(p.first + k)]);
        }
      }
    ...
    for(unsigned int j = 0 ; j < v.size(); j++){
      pair<vector<int>::iterator, vector<long double>::iterator > p = g.undir_neighbors(v[j]);

      for(int k=0 ; k<g.nb_out_neighbors(v[j]) ; k++){
        bool from = (std::find(parts[i].begin(), parts[i].end(), v[j]) != parts[i].end()) && (std::find(parts[i].begin(), parts[i].end(), *(p.first + k)) == parts[i].end());
        bool to = (std::find(parts[i].begin(), parts[i].end(), v[j]) == parts[i].end()) && (std::find(parts[i].begin(), parts[i].end(), *(p.first + k)) != parts[i].end());
        if(from || to){
